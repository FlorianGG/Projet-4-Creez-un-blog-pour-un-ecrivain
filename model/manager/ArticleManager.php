<?php
	...
	class ArticleManager extends ManagerAbstract{
		...
		public function previousId($id){
			...
			$req = $this->bdd->prepare('SELECT * FROM ' . $this->tableName . ' WHERE id<:id ORDER BY id DESC LIMIT 1');
		...
		public function nextId($id){
			...
			$req = $this->bdd->prepare('SELECT * FROM ' . $this->tableName . ' WHERE id>:id ORDER BY id ASC LIMIT 0,1');
